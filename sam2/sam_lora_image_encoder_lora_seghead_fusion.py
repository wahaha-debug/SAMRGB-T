...
class LoRA_Sam(nn.Module):
    ...
    def forward(self, batched_input, multimask_output):
        ...
        _, fc, fh, fw = high_res_0.size()  # 获取输出的形状信息
        high_res_0 = high_res_0.reshape(m, b, fc, fh, fw)  # 恢复原始批次维度的形状
        high_res_0 = torch.mean(high_res_0, dim=0)  # 对维度 m 进行平均，可能是为了合并不同序列的结果
        # print(output.shape)

        _, fc, fh, fw = high_res_1.size()  # 获取输出的形状信息
        high_res_1 = high_res_1.reshape(m, b, fc, fh, fw)  # 恢复原始批次维度的形状
        ...
        return m_output ,output # 返回最终的模型输出
