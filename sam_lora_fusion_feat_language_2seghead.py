...
class PrototypeSegmentation:
    def __init__(self, num_classes, feature_dim):
        ...
        self.global_prototypes = torch.zeros((num_classes, feature_dim), requires_grad=True).to(
            'cuda')  # Set requires_grad=True   #这是一个大小为 (num_classes, feature_dim) 的张量，用来存储每个类别的全局原型。
    ...
    def prototype_loss(self, batch_prototypes):
        ...
        # loss = F.kl_div(F.log_softmax(batch_prototypes, dim=-1), F.softmax(self.global_prototypes, dim=-1),reduction='mean')
        loss = F.kl_div(F.log_softmax(batch_prototypes, dim=-1), F.softmax(self.global_prototypes, dim=-1),reduction='none')
...
class LoRA_Sam(nn.Module):
    ...
    def __init__(self, model_clip, sam_model: SAM2Base, r: int, lora_layer=None):
        ...
        #++++++++++fusion_model+++fusion_model+++fusion_model+++fusion_model+++fusion_model
        num_heads = [1, 2, 4]
        ...
        self.class_names = ['unlabeled', 'car', 'person', 'bike', 'curve', 'car_stop', 'guardrail', 'color_cone',
                            'bump']

        self.prototype_segmentation = PrototypeSegmentation(9, 256)
    ...
    def seg_fuse(self, src, feat_s0, feat_s1):
        ...
        src_transformed = F.interpolate(src_transformed, size=[640, 640], mode='bilinear',
                                        align_corners=False)  # 将特征上采样到固定的分辨率（1024x1024），使用双线性插值。
        ...
        feat_s0_transformed = feat_s0_transformed.transpose(1, 2).view(b, -1, feat_s0.size(2), feat_s0.size(
            3))  # 将变换后的 `feat_s0` 特征转置回原始格式，并恢复为二维空间形状。 8，32，160，160
        ...
        feat_s1_transformed = feat_s1_transformed.transpose(1, 2).view(b, -1, feat_s1.size(2), feat_s1.size(
            3))  # 将变换后的 `feat_s1` 特征转置回原始格式，并恢复为二维空间形状。 #
        ...
        combined_features = torch.cat([src_transformed, feat_s0_transformed, feat_s1_transformed],
                                      dim=1)  # 将 `src_transformed`、`feat_s0_transformed` 和 `feat_s1_transformed` 在通道维度（dim=1）进行拼接。
    ...
    def deeplab_fuseadd(self, feat, high_res_0, high_res_1):
        # 定义DeepLab特征融合方法，输入为深层特征 `feat` 和两层高分辨率特征 `high_res_0` 和 `high_res_1`。
        ...
        # 对拼接后的特征应用一系列3x3卷积进行特征融合。
        # 该操作能够捕获局部上下文信息，并生成融合后的特征图。
        output = self.final_conv_add(fused_features)
        # 对融合后的特征图应用1x1卷积，生成最终的分割结果。
        # 这里的通道数通常对应于分割任务中的类别数。
        output = F.interpolate(output, size=(640, 640), mode='bilinear', align_corners=False)
        # 将输出特征图上采样到最终的目标分辨率（640x640）。
    ...
    def forward(self, batched_input, label, multimask_output):
        ...
        # _, f0c, f0h, f0w = high_res_0.size()  # 获取输出的形状信息
        # high_res_0 = high_res_0.reshape(m, b, f0c, f0h, f0w)  # 恢复原始批次维度的形状
        # high_res_0 = torch.mean(high_res_0, dim=0)  # 对维度 m 进行平均，可能是为了合并不同序列的结果
        # # torch.Size([8, 32, 160, 160])

        high_res_1 = image_embedding["backbone_fpn"][1]  # 获取图像编码器FPN中第二层的高分辨率特征图，
        # _, f1c, f1h, f1w = high_res_1.size()  # 获取输出的形状信息
        # high_res_1 = high_res_1.reshape(m, b, f1c, f1h, f1w)  # 恢复原始批次维度的形状
        # high_res_1 = torch.mean(high_res_1, dim=0)  # 对维度 m 进行平均，可能是为了合并不同序列的结果
        # # torch.Size([8, 64, 80, 80])

        feat = image_embedding['vision_features']  # 获取图像的深层视觉特征，形状为
        # _, fc, fh, fw = feat.size()  # 获取输出的形状信息
        # feat = feat.reshape(m, b, fc, fh, fw)  # 恢复原始批次维度的形状
        # feat = torch.mean(feat, dim=0)  # 对维度 m 进行平均，可能是为了合并不同序列的结果
        ...
        if self.training:
            ...
            # 使用 CLIP 模型编码文本
            feature = self.get_text_feature(text_labels).to(feat.dtype)  # 9 512
            # 计算文本特征的相似度矩阵
            text_sim_matrix = F.cosine_similarity(feature.unsqueeze(0), feature.unsqueeze(1), dim=-1)  # 9x9
            # 获取批次原型和计算原型损失
            batch_prototypes, prototype_loss = self.prototype_segmentation.compute_loss(x_fused, label)  # 9x9
            ...
            kl_loss = torch.tensor(0.0).to(x_fused.device)  # 在验证模式下，不计算KL损失
            prototype_loss = torch.tensor(0.0).to(x_fused.device)  # 在验证模式下，不计算KL损失
        ...
        multi_mask_output = self.sam._forward_sam_heads(
            ...
            # 高分辨率特征，从 FPN 中提取 feat_s0, feat_s1 = high_res_features  # 获取高分辨率特征
        ...
        _, fmc, fmh, fmw = output.size()  # 获取输出的形状信息
        m_output = m_output.reshape(m, b, fmc, fmh, fmw)  # 恢复原始批次维度的形状
